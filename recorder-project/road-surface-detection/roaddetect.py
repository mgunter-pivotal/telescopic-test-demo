...
class PacketReceiverThread(threading.Thread):
        ...
        while not quitflag:
            for (key, events) in selector.select():
                ...
                if key == key_udp_socket:
                    ...
                    #timestamped_packets = TimestampedPacket(timestamp, packet)
                    self.process_incoming_packets(timestamp, packet)
    ...
    def process_incoming_packets(self, timestamp, packet):
        """Process incoming packets by recording them into the correct database file.
        ...
        #ctypes.LittleEndianStructure._objects.get.
        ...
        packet_type = HeaderFieldsToPacketType.get(packet_type_tuple)
        #print(packet_type," <> ", header.packetId)
        if packet_type is None:
            logging.error(
                "Dropped unrecognized packet (format, version, id) = %r.",
                packet_type_tuple,
        ...
        if header.packetId == PacketID.CAR_TELEMETRY:  # Log CAR-Telemetry packets
            telemetry_packet  = unpack_udp_packet(packet)
            telemetry = f1_2020_telemetry.packets.CarTelemetryData_V1.from_buffer_copy(telemetry_packet)
            #print(" packet contents", str(telemetry) )
            #print(telemetry_packet.carTelemetryData," <--")

            #print("car speed:", telemetry.speed)
            #print("car throttle:", telemetry.throttle)

            # logging.info(
            #     "Recording telemetry packet: %s",
            #     telemetry_packet.__str__,
            # )

        if header.packetId == PacketID.MOTION:  # Log CAR-Telemetry packets
            telemetry_packet  = unpack_udp_packet(packet)
            motion = f1_2020_telemetry.packets.PacketMotionData_V1.from_buffer_copy(telemetry_packet)
            #print(" packet contents", str(motion) )
            #print(telemetry_packet.carTelemetryData," <--")

            #print("wheel slip:", motion.wheelSlip[0])
            #print("wheel speed:", motion.wheelSpeed[0])
        ...
        if header.packetId == PacketID.PARTICIPANTS:
            participant_packet =  unpack_udp_packet(packet)
            participants = ParticipantData_V1.from_buffer_copy(participant_packet)
            #for p in participants
            print( "driver ID", participants.driverId )
            print( "driver ID", participants.yourTelemetry )
        ...
        #print("sessionpacket car index", session_packet.playerCarIndex, "\n")
...
def main():
    ...
    receiver_thread = PacketReceiverThread()
